...
from inclearn.loss.tripeloss import TripletLossNoHardMining, TripletLoss
...
from inclearn.loss.focalloss import BCEFocalLoss, MultiCEFocalLoss
from inclearn.models.prune import AverageMeter, Mask
...
class IncModel(IncrementalLearner):
    ...
    def _forward_loss(self, inputs, targets, old_classes, new_classes, epoch, accu=None, new_accu=None, old_accu=None):
        inputs, targets = inputs.to(self._device, non_blocking=True), targets.to(self._device, non_blocking=True)

        outputs = self._parallel_network(inputs)
        if accu is not None:
            accu.add(outputs['logit'], targets)
        return self._compute_loss(inputs, targets, outputs, old_classes, new_classes, epoch)
    ...
    def _compute_loss(self, inputs, targets, outputs, old_classes, new_classes, epoch):
        ...
        if self._cfg["distillation"] and self._old_model is not None:
            outputs_old = self._old_model(inputs)
            ...
            if self._cfg["disttype"] == "KL":
                ...
                if modify:
                    old_weight_norm = torch.norm(self._network.classifier.weight[:-self._task_size], p=2, dim=1)
                    new_weight_norm = torch.norm(self._network.classifier.weight[-self._task_size:], p=2, dim=1)
            ...
            elif self._cfg["disttype"] == "KRaKL":
                targets_old1 = targets_old
                log_probs_new = (outputs['logit'][:, :-self._task_size] / self._temperature).log_softmax(dim=1)
                if self._task > 1 and self._cfg["postprocessor"]["enable"]:
                    if self._cfg["postprocessor"]["type"].lower() == "aver":
                        targets_old1 = self._old_model.module.postprocessor.post_process(targets_old1, self._task_size, self.classnum_list[:-1], self._task-1)
                    else:
                        targets_old1 = self._old_model.module.postprocessor.post_process(targets_old1, self._task_size)
                modify =  self._cfg.get("modify_new",False)
                if modify:
                    old_weight_norm = torch.norm(self._network.classifier.weight[:-self._task_size], p=2, dim=1)
                    new_weight_norm = torch.norm(self._network.classifier.weight[-self._task_size:], p=2, dim=1)
                    ...
                    targets_old1[new_classes,:] = targets_old1[new_classes,:] * gamma
                probs_old = (targets_old1 / self._temperature).softmax(dim=1)
                
                kl_loss = F.kl_div(log_probs_new, probs_old, reduction="batchmean")
                ...
                if self._task > 1 and self._cfg["postprocessor"]["enable"]:
                    if self._cfg["postprocessor"]["type"].lower() == "aver":
                        ...
                        targets_old = self._old_model.module.postprocessor.post_process(targets_old, self._task_size)
                log_probs_new = outputs['logit'][:, :-self._task_size]

                ignore =  self._cfg.get("ignore_new",False)
                
                s_ft = outputs['features'][-1]
                t_ft = outputs_old['features']

                if ignore:
                    old_classes_indice = torch.from_numpy(np.where(old_classes==True)[0]).to(self._device)
                    targets_old = torch.index_select(targets_old, 0, old_classes_indice)
                    log_probs_new = torch.index_select(log_probs_new, 0, old_classes_indice)
                    s_ft = torch.index_select(s_ft, 0, old_classes_indice)
                    t_ft = [torch.index_select(ft, 0, old_classes_indice) for ft in t_ft]

                s_features, pred = s_ft, log_probs_new
                t_features, t_pred = t_ft, targets_old

                feature_kd_loss = self.hcl(s_features, t_features, targets)
                # feature_kd_loss = feature_kd_loss / ((feature_kd_loss/div_loss).detach())
                dist_loss = feature_kd_loss * min(1, epoch/self._cfg["attention"]["kd_warm_up"]) * self._cfg["attention"]["kd_loss_weight"]
                if self._cfg["attention"]["add_kl"]:
                    dist_loss += kl_loss * self._cfg["attention"]["kl_loss_weight"]

            elif self._cfg["disttype"] == "MTKD":
                #kl_criterion = DistillKL(self._temperature)
                kl_criterion = DistillKL_MLD()
                if self._task > 1 and self._cfg["postprocessor"]["enable"]:
                    if self._cfg["postprocessor"]["type"].lower() == "aver":
                        targets_old = self._old_model.module.postprocessor.post_process(targets_old, self._task_size, self.classnum_list[:-1], self._task-1)
                    else:
                ...
                dist_loss = feature_kd_loss * min(1, epoch/self._cfg["attention"]["kd_warm_up"]) * self._cfg["attention"]["kd_loss_weight"]
                if self._cfg["attention"]["add_kl"]:
                    dist_loss += kl_criterion(pred, t_pred) * self._cfg["attention"]["kl_loss_weight"]
    ...
    def _update_postprocessor(self, inc_dataset):
        if self._cfg["postprocessor"]["type"].lower() == "bic":
            ...
            self._network.postprocessor.update(self._network.classifier, self._task_size, self.classnum_list, self._task)
...
def er_kd_loss(logits_student, logits_teacher, temperature):
...
class DistillKL_easy(nn.Module):
    def __init__(self, T):
        super(DistillKL_easy, self).__init__()
        self.T = T

    def forward(self, y_s, y_t):
        er_kd_loss1 = er_kd_loss(y_s, y_t, self.T)
        #cc_loss1 = cc_loss(y_s, y_t, self.T)
        bc_loss1 = bc_loss(y_s, y_t, self.T) 
        #loss = er_kd_loss1 + cc_loss1 + bc_loss1
        loss = er_kd_loss1 + bc_loss1
        return loss


#MLD_logits蒸馏
class DistillKL_MLD(nn.Module):
    def __init__(self):
        super(DistillKL_MLD, self).__init__()
        self.temperature = 4
        self.t = 4
        self.er = True
    ...
    def forward(self, y_s, y_t):
        logit_student = y_s
        logit_teacher = y_t
        batch_size, class_num = logit_student.shape

        pred_teacher = F.softmax(logit_teacher.detach(), dim=1)
